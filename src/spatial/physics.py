...
def calculate_steering_vector(mic_positions: np.ndarray, source_direction: np.ndarray) -> np.ndarray:
    """
    Calculates the relative path length differences for a plane wave arriving from a specific direction.
    ...
    Returns:
        (N_mics,) array of distance differences relative to the array center (or reference).
        Positive distance means the wavefront arrives *sooner* (source is closer).
    """
    # Project mic positions onto the source vector
    # If source is at infinity (plane wave assumption)
    # Distance = dot(mic_pos, source_vec)
    # Note: This is the distance *along the direction of propagation*.
    return np.dot(mic_positions, source_direction)

def azimuth_elevation_to_vector(azimuth_deg: float, elevation_deg: float = 0.0) -> np.ndarray:
    """
...
def apply_subsample_shifts(signal: np.ndarray, delays_seconds: np.ndarray, sample_rate: int) -> np.ndarray:
    """
